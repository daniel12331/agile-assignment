...
const MovieList = ( {actors, movies, action, title, credits, type}) => {
  ...
  let actoreCredits = credits?.cast.map((m) => (
    ...
    return type === 1 || type === 0 || title === 'Discover Movies' || title === 'Upcoming Movies' || title === 'Popular TV Shows' || title === 'Favorite Movies'? movieCards: title === 'Credits' ? actoreCredits: title === "Most Popular Actors" || type === 2? actorCards : ''
